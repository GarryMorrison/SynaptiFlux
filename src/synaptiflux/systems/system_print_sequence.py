...
def system_symbol_sequence(name, symbol_sequence, punctuation_symbols):
    ...
    NM_print_symbols = module_print_symbols_to_buffer.module_print_symbols_to_buffer('print symbols module', sorted(symbol_set), infix_str = "")
    ...
    for k in range(seq_len):
        ...
        if symbol_sequence[k].isupper():
            inputs_list.append([channel, "use capitals"])
        inputs_list.append([channel, neuron])
    flush_channel = f"!seq-{seq_len + 1}!"
    inputs_list.append([flush_channel, "flush buffer"]) # be sure to flush our buffer!
